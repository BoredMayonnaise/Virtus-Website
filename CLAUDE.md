...
- `/admin` and all non-public `/api/*` need `admin`; `/team` accepts `team` or `admin`. Middleware checks the cookie signature; `getStaff()` in `src/lib/staffAuth.ts` re-checks the account (disabled, signed out) in pages and handlers.
